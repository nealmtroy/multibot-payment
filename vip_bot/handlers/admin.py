...
def register_admin_handlers(client, config, store, qris_semaphore, user_locks, bot_manager=None):
    # -------------------------------------------------------------------------
    ...
    async def bot_add(event):
        ...
        try:
            ...
            await status_msg.edit(
                ...
                "Bot siap menerima pembayaran! Gunakan <code>/package_add</code> untuk menambahkan group VIP ke bot ini.",
                parse_mode="html",
            )
            await send_log(
                client,
    ...
    async def bot_start(event):
        ...
        if not bot_code:
            ...
            return
        bot_data = store.get_bot(bot_code)
        if not bot_data:
        ...
        try:
            ...
            await send_log(client, config, store, f"<b>Bot Started</b>\nCode: <code>{html.escape(bot_code)}</code>\nAdmin: <code>{event.sender_id}</code>")
        except Exception as exc:
    ...
    async def package_add(event):
        ...
        try:
            ...
            store.upsert_package(code, name, vip_chat_id, amount, config.invite_expire_hours, bot_code=bot_code)
            await event.respond(
    ...
    async def package_list(event):
        ...
        packages = store.list_all_packages(bot_code=bot_code)
    ...
    async def package_delete(event):
        ...
        if not raw:
            await event.respond("Format: <code>/package_delete &lt;nama_bot&gt; &lt;kode&gt;</code> atau <code>/package_delete &lt;kode&gt;</code>", parse_mode="html")
            return
        tokens = raw.split()
        if len(tokens) >= 2:
        ...
        try:
            ...
            changed = store.delete_package(normalized, bot_code=bot_code)
            if changed:
    ...
    async def withdrawal_admin_action(event):
        ...
        try:
            ...
            # Send notification via the bot that received the request
            ...
            await send_log(client, config, store, f"<b>Withdrawal admin action error</b>\nID: <code>{html.escape(withdrawal_id)}</code>\n<code>{html.escape(str(exc))}</code>")
    ...
    async def custom_payment(event):
        ...
        from vip_bot.handlers.admin import send_custom_qris
